...
if(empty($data->id) || empty($data->nombre) || empty($data->ruc)) {
    echo json_encode(['error' => 'ID, Nombre y RUC son requeridos']);
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("UPDATE empresa SET nombre=?, ruc=?, direcion=?, telefono=?, email=?, horario_abierto_entre_semana=?, horario_cerrado_entre_semana=?, horario_abierto_fin_semana=?, horario_cerrado_fin_semana=?, logo=?, link_facebook=?, link_instagram=?, link_tiktok=? WHERE id=?");
    $stmt->execute([
        $data->nombre,
        $data->ruc,
        $data->direcion ?? null,
    ...
    echo json_encode(['message' => 'Empresa actualizada con éxito']);
} catch (Exception $e) {
